...
SELECT 
    COUNT(DISTINCT id) AS no_of_movies, year
FROM
    movie
WHERE
    year = 2019
...
; 
-- USA and India produced 1059 movies.
...
SELECT 
    COUNT(DISTINCT movie_id) AS no_of_movies, genre
FROM
    genre
GROUP BY genre;
...
INNER JOIN top_3_genres t ON t.genre = g.genre
...
SELECT name as actress_name, SUM(total_votes) AS total_votes, COUNT(rm.movie_id) as movie_id, Round(Sum(avg_rating*total_votes)/Sum(total_votes),2) AS actress_avg_rating,
